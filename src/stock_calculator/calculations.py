...
class Calculate:
    def returns(self, history_df: pd.DataFrame, input_df: pd.DataFrame):
        ...
        :parameters
            history_df: DataFrame -> dataframe containing the historical close prices.
            input_df: DataFrame -> dataframe containing the details about stocks in portfolio.
        """
        averaged_returns = history_df["Close"].pct_change().mean()

        temp_df = pd.DataFrame(
            {"Stock": averaged_returns.index.values, "Return": averaged_returns.values}
        )
